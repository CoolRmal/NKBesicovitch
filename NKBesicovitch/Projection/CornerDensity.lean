...
noncomputable def cornerDensity (a b c κ : ℝ) (D : Set (CornerCoordinates m))
    (z : Space m) : ℝ≥0∞ :=
  codeJacobian m a b ^ 2 * ∫⁻ p : Line m × Space m, D.indicator 1 (cornerFromCode a b c κ p.1 p.2 z)

/-- First lines represented on a corner-code fiber. -/
def cornerFiber (a b c κ : ℝ) (D : Set (CornerCoordinates m)) (z : Space m) : Set (Line m) :=
...
noncomputable def positiveCornerFiber (a b c κ : ℝ) (D : Set (CornerCoordinates m))
    (z : Space m) : Set (Line m) := Function.support (cornerFirstDensity a b c κ D z)

theorem measurable_cornerFirstDensity (a b c κ : ℝ) {D : Set (CornerCoordinates m)}
    (hD : MeasurableSet D) (z : Space m) : Measurable (cornerFirstDensity a b c κ D z) := by
  have h : Measurable (fun p : Line m × Space m ↦
...
theorem measurableSet_positiveCornerFiber (a b c κ : ℝ) {D : Set (CornerCoordinates m)}
    (hD : MeasurableSet D) (z : Space m) : MeasurableSet (positiveCornerFiber a b c κ D z) :=
  measurableSet_support (measurable_cornerFirstDensity a b c κ hD z)

theorem positiveCornerFiber_subset (a b c κ : ℝ) (D : Set (CornerCoordinates m)) (z : Space m) :
...
theorem measurable_cornerDensity (a b c κ : ℝ) {D : Set (CornerCoordinates m)}
  ...
  have h : Measurable (fun p : (Line m × Space m) × Space m ↦
...
theorem lintegral_cornerDensity {a b : ℝ} (hab : a ≠ b) (c κ : ℝ)
  ...
  let f : (Line m × Space m) × Space m → ℝ≥0∞ := fun p ↦
  ...
  calc
    ...
    _ = codeJacobian m a b ^ 2 * ∫⁻ p, f p := by
      ...
      simpa only [Measure.volume_eq_prod] using
        (lintegral_prod_symm' (μ := (volume : Measure (Line m × Space m)))
...
theorem mem_cornerFiber_iff {a b : ℝ} (hab : a ≠ b) (c κ : ℝ)
    (D : Set (CornerCoordinates m)) (g : Line m) (z : Space m) :
...
theorem cornerFiber_subset (a b c κ : ℝ) {G : Set (Line m)} {D : Set (CornerCoordinates m)}
    (hDG : D ⊆ cornerFamily a b G) (z : Space m) : cornerFiber a b c κ D z ⊆ G := by
  rintro g ⟨x, hx⟩
  have h := (hDG hx).1
