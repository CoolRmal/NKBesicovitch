...
theorem exists_outer_constant (P : CornerPattern m β) (hβ : 1 < β) (hβ2 : β ≤ 2) :
    ∃ C : ℝ, 0 < C ∧ ∀ G : Set (Line m), IsBounded G → (parallelMultiplicity G).toReal ≤ 1 →
      ∀ N R : ℝ≥0∞, N ≠ 0 → N ≠ ∞ → R ≠ 0 → R ≠ ∞ →
  ...
  refine ⟨A ^ (1 / (β - 1)), Real.rpow_pos_of_pos hA _,
    fun G hGb hM N R hN₀ hN hR₀ hR hcN huN D hD hDG hpos W E hDW hDE Q₀ hQ₀ hQ himage ↦ ?_⟩
