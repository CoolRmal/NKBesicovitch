...
def Grassmannian (n k : ℕ) := {V : Submodule ℝ (Space n) // Module.finrank ℝ V = k}

namespace Grassmannian

variable {n k : ℕ}

/-- The underlying linear subspace of a direction. -/
abbrev submodule (V : Grassmannian n k) : Submodule ℝ (Space n) := V.val

/-- The orthogonal projection onto a direction, viewed as an ambient operator. -/
noncomputable def projection (V : Grassmannian n k) : Space n →L[ℝ] Space n :=
  V.submodule.starProjection
...
theorem projection_injective : Function.Injective (projection (n := n) (k := k)) := by
  intro V W h
  apply Subtype.ext
  have hr := congrArg (fun p : Space n →L[ℝ] Space n ↦ LinearMap.range p.toLinearMap) h
