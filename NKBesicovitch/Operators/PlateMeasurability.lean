...
theorem plate_average_indicator_eq (δ : ℝ) (u : Rotations n) (a : EuclideanSpace ℝ (Fin n))
    (V : Grassmannian n k) {U : Set (EuclideanSpace ℝ (Fin n))} (hU : MeasurableSet U) :
    (∫⁻ x in plate δ (Grassmannian.rotate u V).val a,
        U.indicator (fun _ ↦ (1 : ℝ≥0∞)) x) /
      volume (plate δ (Grassmannian.rotate u V).val a) =
    ∫⁻ x in plate δ V.val 0,
      U.indicator (fun _ ↦ (volume (plate δ V.val 0))⁻¹) (rigidMotion u a x) := by
  ...
  have hi := (measurePreserving_rigidMotion u a).setLIntegral_comp_preimage
    (s := plate δ (Grassmannian.rotate u V).val a)
  ...
  rw [← hi, volume_plate_rotate, div_eq_mul_inv,
    ← lintegral_mul_const (μ := volume.restrict (plate δ V.val 0)) _ hm']
  simp only [← Set.indicator_mul_const, one_mul]

theorem lowerSemicontinuous_plate_average_orbit (δ : ℝ) (a : EuclideanSpace ℝ (Fin n))
    (V : Grassmannian n k) {U : Set (EuclideanSpace ℝ (Fin n))} (hU : IsOpen U) :
  ...
  · intro x
    ...
    change Continuous (fun u : Rotations n ↦ (u : EuclideanSpace ℝ (Fin n) →L[ℝ]
