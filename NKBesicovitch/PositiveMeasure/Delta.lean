...
theorem volume_plate_pos {δ : ℝ} (hδ : 0 < δ) (V : Submodule ℝ (Space n)) (a : Space n) :
...
theorem volume_plate_lt_top (δ : ℝ) (V : Submodule ℝ (Space n)) (a : Space n) :
    volume (plate δ V a) < ⊤ :=
...
theorem exists_scale_one_le_plateMaximal {U : Set (Space n)} (hU : IsOpen U)
    (hB : IsBesicovitch k U) (V : Grassmannian n k) :
    ∃ δ₀ > 0, ∀ δ ∈ Ioc (0 : ℝ) δ₀,
      1 ≤ plateMaximal δ (U.indicator (fun _ ↦ (1 : ℝ≥0∞))) V := by
  obtain ⟨a, ha⟩ := isBesicovitch_iff_unitDisk_subset.mp hB V.submodule V.property
  obtain ⟨δ₀, hδ₀, hsub⟩ := (isCompact_unitDisk V.submodule a).exists_thickening_subset_open hU ha
  refine ⟨δ₀, hδ₀, fun δ hδ ↦ ?_⟩
  have hs : plate δ V.submodule a ⊆ U := (Metric.thickening_mono hδ.2 _).trans hsub
  have he : (∫⁻ x in plate δ V.submodule a, U.indicator (fun _ ↦ (1 : ℝ≥0∞)) x) =
      volume (plate δ V.submodule a) := by
    have hg : ∀ x ∈ plate δ V.submodule a, U.indicator (fun _ ↦ (1 : ℝ≥0∞)) x = 1 :=
      fun x hx ↦ Set.indicator_of_mem (hs hx) _
    rw [setLIntegral_congr_fun (μ := volume) (s := plate δ V.submodule a)
      Metric.isOpen_thickening.measurableSet hg,
      setLIntegral_const, one_mul]
  have hv : (∫⁻ x in plate δ V.submodule a, U.indicator (fun _ ↦ (1 : ℝ≥0∞)) x) /
      volume (plate δ V.submodule a) = 1 := by
    rw [he]
